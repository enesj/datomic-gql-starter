(ns hodur.db.spec-schema
  (:require [clojure.spec.alpha :as s]
            [datascript.core :as d]
            [datascript.query-v3 :as q]
            [camel-snake-kebab.core :refer [->kebab-case-string]]))

"req-un replaced with req, get-spec-field-name modified "
...
(defn ^:private get-spec-field-name
  ...
  (keyword (->kebab-case-string type-name)
           (->kebab-case-string field-name)))
...
(defmethod get-spec-form* :enum
  ...
  "Modified by Enes -> (hash-set (get-spec-name field opts))..."
  (list* `s/or
         (reduce (fn [c {:keys [field/kebab-case-name] :as field}]
                   (conj c kebab-case-name (hash-set
                                             (get-spec-name field opts)
                                             (keyword (name (get-spec-name field opts))))))
                 [] _parent)))
