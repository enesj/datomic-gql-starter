...
(defn get-data [gql-query]
  ;(val (first (:data (:body (send-request (get-gql-query (composition-to-string gql-query)))))))
  (-> gql-query
...
(def artist-releases-tracks-media-duration-filter
  "Or tracks duration that are bigger then 10000 and media has 7,8 or 9 tracks, of every release per artist"
  [[:Artists {:name   "Bill Cosby"}
    [:name
     [:_releases
      [:name :year
